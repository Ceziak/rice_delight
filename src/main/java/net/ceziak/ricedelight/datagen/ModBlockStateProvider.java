...
import net.ceziak.ricedelight.block.BellPepperCropBlock;
...
public class ModBlockStateProvider extends BlockStateProvider {
    public ModBlockStateProvider(PackOutput output, ExistingFileHelper exFileHelper) {
        super(output, RiceDelight.MOD_ID, exFileHelper);
    }
    ...
    private ConfiguredModel[] bellPepperStates(BlockState state, CropBlock block, String modelName, String textureName) {
        ...
        models[0] = new ConfiguredModel(models().crop(modelName + state.getValue(((BellPepperCropBlock) block).getAgeProperty()),
                new ResourceLocation(RiceDelight.MOD_ID, "block/" + textureName + state.getValue(((BellPepperCropBlock) block).getAgeProperty()))).renderType("cutout"));
